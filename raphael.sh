...
setversioning() {
    # For staging branchc
	if [ $LTO = "1" ];then
	    KERNELNAME="ResurgenceKernel-$COMPILER-LTO-$DATE"
	else 
	    KERNELNAME="ResurgenceKernel-$COMPILER-$DATE"
	fi
...
build_kernel() {
	...
	then
		...
		python2 "$KERNEL_DIR/scripts/ufdt/libufdt/utils/src/mkdtboimg.py" \
			create "$KERNEL_DIR/out/arch/arm64/boot/dtbo.img" --page_size=4096 "$KERNEL_DIR/out/arch/arm64/boot/dts/qcom/raphael-sm8150-overlay.dtbo"
	fi
}
